<x-guest-layout>
    ...
    <div class="absolute inset-0 flex items-center justify-center">
        <div class="h-3/6 w-8/12 bg-white-500 rounded-md bg-clip-padding backdrop-filter backdrop-blur-xl bg-opacity-30">
            <div class="w-full h-full flex flex-col justify-center items-center my-4">
                <img src="{{ asset('storage/images/logo.png') }}" alt="" class="w-3/12 h-4/6">
                @if (Route::has('login'))
                    @auth
                        <div class="w-3/12 mt-8 flex justify-center">
                            <x-btn-basic url="/dashboard" label="Ir al inicio" />
                        </div>
                    @else
                        <div class="w-3/12 mt-8 flex justify-around">
